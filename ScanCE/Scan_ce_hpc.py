...
def ce_caller(bamfile, referencename, chrm, stranded = 'no', mapq = 50):

    known_splices_D = defaultdict(list)
    known_splices_A = defaultdict(list)
    gtf = pysam.TabixFile(referencename, parser = pysam.asGTF())
    gtf2 = pysam.TabixFile("/panfs/home/yang4414/lu000016/ref/GRCh38_latest_genomic.sorted.gff.gz", parser = pysam.asGTF())
    db = gffutils.FeatureDB(referencename + '.db')
    db2 = gffutils.FeatureDB("/panfs/home/yang4414/lu000016/ref/GRCh38_latest_genomic.gff.gz.db")
    ...
    introns, reads = find_introns(
       (read for read in bamfile.fetch(chrm) if read .mapping_quality >= 50),  
        stranded
        )
